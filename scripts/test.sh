...
echo "[5/11] timer interrupt check"
make clean
make TIMER_TEST=1
...
echo "[6/11] heap allocator check"
make clean
make HEAP_TEST=1
...
echo "[8/11] keyboard decoder check"
make clean
...
echo "[9/11] virtual memory stack check"
make clean
make VM_TEST=1
run_qemu vm_log.txt
...
echo "[10/11] physical memory manager check"
make clean
make PMM_TEST=1
...
echo "[11/11] virtual memory mapping check"
make clean
make VMM_TEST=1
