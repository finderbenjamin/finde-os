...
echo "[5/39] timer interrupt check"
make clean
make TIMER_TEST=1
...
echo "[6/39] heap allocator check"
make clean
make HEAP_TEST=1
...
echo "[8/39] keyboard decoder check"
make clean
...
echo "[9/39] virtual memory stack check"
make clean
make VM_TEST=1
run_qemu vm_log.txt
...
echo "[10/39] physical memory manager check"
make clean
make PMM_TEST=1
...
echo "[11/39] virtual memory mapping check"
make clean
make VMM_TEST=1
...
echo "[12/39] page fault handler check"
make clean
make PF_TEST=1
run_qemu pf_log.txt
...
echo "[13/39] nx execute protection check"
make clean
make NX_TEST=1
run_qemu nx_log.txt
...
echo "[15/39] shell line editing check"
make clean
make EDIT_TEST=1
...
echo "[16/39] capability primitives check"
make clean
make CAP_TEST=1
...
echo "[17/39] capability enforcement check"
make clean
...
echo "[18/39] syscall capability gate check"
make clean
make SYSCALL_TEST=1
...
echo "[19/39] syscall deny path check"
make clean
...
echo "[20/39] task scheduler check"
make clean
make TASK_TEST=1
...
echo "[21/39] task capability isolation check"
make clean
...
echo "[22/39] capability generation guard check"
make clean
make CAP_GEN_TEST=1
...
echo "[23/39] user mode launch path check"
make clean
...
echo "[24/39] user task capability path check"
make clean
...
echo "[25/39] user task capability deny path check"
make clean
...
echo "[26/39] user-space driver isolation check"
make clean
make DRV_ISO_TEST=1
...
echo "[27/39] microvm isolation smoke check"
make clean
make MICROVM_TEST=1
...
echo "[28/39] microvm capability boundary check"
make clean
...
echo "[29/39] inter-vm capability isolation check"
make clean
make INTERVM_TEST=1
...
echo "[30/39] ipc capability channel check"
make clean
make IPC_TEST=1
...
echo "[31/39] capability revocation propagation check"
make clean
make REVOKE_TEST=1
...
echo "[32/39] syscall rate guard check"
make clean
...
echo "[33/39] process/task resource quota check"
make clean
make QUOTA_TEST=1
...
echo "[34/39] capability type isolation matrix check"
make clean
...
echo "[35/39] capability lifecycle check"
make clean
...
echo "[36/39] ipc + inter-vm platform hardening check"
make clean
...
echo "[37/39] limits telemetry + deny reason check"
make clean
make LIMITS_TEST=1
...
echo "[38/39] usermode + driver path hardening check"
make clean
...
echo "[39/39] microvm golden security mode boundary check"
make clean
