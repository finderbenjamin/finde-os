...
echo "[5/21] timer interrupt check"
make clean
make TIMER_TEST=1
...
echo "[6/21] heap allocator check"
make clean
make HEAP_TEST=1
...
echo "[8/21] keyboard decoder check"
make clean
...
echo "[9/21] virtual memory stack check"
make clean
make VM_TEST=1
run_qemu vm_log.txt
...
echo "[10/21] physical memory manager check"
make clean
make PMM_TEST=1
...
echo "[11/21] virtual memory mapping check"
make clean
make VMM_TEST=1
...
echo "[12/21] page fault handler check"
make clean
make PF_TEST=1
run_qemu pf_log.txt
...
echo "[13/21] nx execute protection check"
make clean
make NX_TEST=1
run_qemu nx_log.txt
...
echo "[15/21] shell line editing check"
make clean
make EDIT_TEST=1
...
echo "[16/21] capability primitives check"
make clean
make CAP_TEST=1
...
echo "[17/21] capability enforcement check"
make clean
...
echo "[18/21] syscall capability gate check"
make clean
make SYSCALL_TEST=1
...
echo "[19/21] task scheduler check"
make clean
make TASK_TEST=1
...
echo "[20/21] task capability isolation check"
make clean
...
echo "[21/21] capability generation guard check"
make clean
make CAP_GEN_TEST=1
