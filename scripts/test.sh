...
echo "[5/26] timer interrupt check"
make clean
make TIMER_TEST=1
...
echo "[6/26] heap allocator check"
make clean
make HEAP_TEST=1
...
echo "[8/26] keyboard decoder check"
make clean
...
echo "[9/26] virtual memory stack check"
make clean
make VM_TEST=1
run_qemu vm_log.txt
...
echo "[10/26] physical memory manager check"
make clean
make PMM_TEST=1
...
echo "[11/26] virtual memory mapping check"
make clean
make VMM_TEST=1
...
echo "[12/26] page fault handler check"
make clean
make PF_TEST=1
run_qemu pf_log.txt
...
echo "[13/26] nx execute protection check"
make clean
make NX_TEST=1
run_qemu nx_log.txt
...
echo "[15/26] shell line editing check"
make clean
make EDIT_TEST=1
...
echo "[16/26] capability primitives check"
make clean
make CAP_TEST=1
...
echo "[17/26] capability enforcement check"
make clean
...
echo "[18/26] syscall capability gate check"
make clean
make SYSCALL_TEST=1
...
echo "[19/26] syscall deny path check"
make clean
...
echo "[20/26] task scheduler check"
make clean
make TASK_TEST=1
...
echo "[21/26] task capability isolation check"
make clean
...
echo "[22/26] capability generation guard check"
make clean
make CAP_GEN_TEST=1
...
echo "[23/26] user mode launch path check"
make clean
...
echo "[24/26] user task capability path check"
make clean
...
echo "[25/26] user task capability deny path check"
make clean
...
echo "[26/26] user-space driver isolation check"
make clean
make DRV_ISO_TEST=1
