...
echo "[5/24] timer interrupt check"
make clean
make TIMER_TEST=1
...
echo "[6/24] heap allocator check"
make clean
make HEAP_TEST=1
...
echo "[8/24] keyboard decoder check"
make clean
...
echo "[9/24] virtual memory stack check"
make clean
make VM_TEST=1
run_qemu vm_log.txt
...
echo "[10/24] physical memory manager check"
make clean
make PMM_TEST=1
...
echo "[11/24] virtual memory mapping check"
make clean
make VMM_TEST=1
...
echo "[12/24] page fault handler check"
make clean
make PF_TEST=1
run_qemu pf_log.txt
...
echo "[13/24] nx execute protection check"
make clean
make NX_TEST=1
run_qemu nx_log.txt
...
echo "[15/24] shell line editing check"
make clean
make EDIT_TEST=1
...
echo "[16/24] capability primitives check"
make clean
make CAP_TEST=1
...
echo "[17/24] capability enforcement check"
make clean
...
echo "[18/24] syscall capability gate check"
make clean
make SYSCALL_TEST=1
...
echo "[19/24] syscall deny path check"
make clean
...
echo "[20/24] task scheduler check"
make clean
make TASK_TEST=1
...
echo "[21/24] task capability isolation check"
make clean
...
echo "[22/24] capability generation guard check"
make clean
make CAP_GEN_TEST=1
...
echo "[23/24] user mode launch path check"
make clean
...
echo "[24/24] user task capability path check"
make clean
