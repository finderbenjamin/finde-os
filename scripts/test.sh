...
echo "[4/8] timer interrupt check"
make clean
make TIMER_TEST=1
...
echo "[7/8] keyboard decoder check"
make clean
...
echo "[8/8] virtual memory stack check"
make clean
make VM_TEST=1
run_qemu vm_log.txt

if ! tr -d '\r' < vm_log.txt | grep -Fxq "VM_OK"; then
