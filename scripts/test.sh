...
echo "[5/22] timer interrupt check"
make clean
make TIMER_TEST=1
...
echo "[6/22] heap allocator check"
make clean
make HEAP_TEST=1
...
echo "[8/22] keyboard decoder check"
make clean
...
echo "[9/22] virtual memory stack check"
make clean
make VM_TEST=1
run_qemu vm_log.txt
...
echo "[10/22] physical memory manager check"
make clean
make PMM_TEST=1
...
echo "[11/22] virtual memory mapping check"
make clean
make VMM_TEST=1
...
echo "[12/22] page fault handler check"
make clean
make PF_TEST=1
run_qemu pf_log.txt
...
echo "[13/22] nx execute protection check"
make clean
make NX_TEST=1
run_qemu nx_log.txt
...
echo "[15/22] shell line editing check"
make clean
make EDIT_TEST=1
...
echo "[16/22] capability primitives check"
make clean
make CAP_TEST=1
...
echo "[17/22] capability enforcement check"
make clean
...
echo "[18/22] syscall capability gate check"
make clean
make SYSCALL_TEST=1
...
echo "[19/22] task scheduler check"
make clean
make TASK_TEST=1
...
echo "[20/22] task capability isolation check"
make clean
...
echo "[21/22] capability generation guard check"
make clean
make CAP_GEN_TEST=1
...
echo "[22/22] user mode launch path check"
make clean
